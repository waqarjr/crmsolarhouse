...
export default function NotesSection() {
  ...
  const handleAdd = () => {
    if (inputValue.trim()) {
      setNewItems([...newItems, { text: inputValue, important: isImportant }]);
      setInputValue('');
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.get('/api/conditions', {
        params: { sectionType: 'notes' }
      });

      const data = response.data.data;

      if (data && data.length > 0) {
        const notesData = data[0].notes;
        
        if (typeof notesData === 'string') {
          setSavedItems(JSON.parse(notesData));
        } else if (Array.isArray(notesData)) {
          setSavedItems(notesData);
        } else {
          setSavedItems([]);
        }
      } else {
          ...
          <ul className="space-y-2">
            {newItems.map((item, index) => (
              <li key={`new-${index}`} className="flex items-start justify-between bg-yellow-50 p-2 rounded border border-yellow-200">
                <p className={`text-sm ${item.important ? 'text-red-600 font-bold' : 'text-gray-700'}`}>
                  {item.text}
                </p>
                <button
