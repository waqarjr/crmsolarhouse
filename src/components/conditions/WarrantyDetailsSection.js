...
export default function WarrantyDetailsSection() {
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.get('/api/conditions', {
        params: { sectionType: 'warrantyDetails' }
      });

      const data = response.data.data;

      if (data && data.length > 0) {
        const warrantyData = data[0].warrantyDetails;
        
        if (typeof warrantyData === 'string') {
          setSavedItems(JSON.parse(warrantyData));
        } else if (Array.isArray(warrantyData)) {
          setSavedItems(warrantyData);
        } else {
          setSavedItems([]);
        }
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Error saving warranty details:', error);
  ...
  return (
    <SectionCard title="Warranty Details">
