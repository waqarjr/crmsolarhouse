...
export default function BehalfOfSection() {
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.get('/api/conditions', {
        params: { sectionType: 'behalfOf' }
      });

      const data = response.data.data;

      if (data && data.length > 0) {
        const behalfOfData = data[0].behalfOf;
        
        if (typeof behalfOfData === 'string') {
          setSavedItems(JSON.parse(behalfOfData));
        } else if (Array.isArray(behalfOfData)) {
          setSavedItems(behalfOfData);
        } else {
          setSavedItems([]);
        }
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Error saving behalf of data:', error);
