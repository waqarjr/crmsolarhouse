  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.get('/api/conditions', {
        params: { sectionType: 'termsConditions' }
      });

      const data = response.data.data;

      if (data && data.length > 0) {
        const termsData = data[0].termsConditions;
        
        if (typeof termsData === 'string') {
          setSavedItems(JSON.parse(termsData));
        } else if (Array.isArray(termsData)) {
          setSavedItems(termsData);
        } else {
          setSavedItems([]);
        }
      } else {
